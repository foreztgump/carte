(function ($, document) {
  ...
  var orderable_addons = {
    ...
    on_ready() {
      ...
      $(document).on('orderable_variation_set', '.orderable-product__add-to-order', function (e, variation_data) {
        if (!variation_data.variation_id) {
          return;
        }
        const $field_group = $(this).closest('.orderable-drawer').find('.orderable-product-fields-group');
        if (!$field_group.length) {
          return;
        }
        const $field_group_wrap = $field_group.closest('.orderable-product-fields-group-wrap');
        $field_group_wrap.data('regular-price', variation_data.variation.display_price);
        $field_group_wrap.data('price', variation_data.variation.display_price);
        orderable_addons.update_price($('.orderable-product-fields').eq(0));
      });
    ...
    select_deselect_visual_field(_this) {
      const this_field = _this instanceof Element ? $(_this) : $(this);
      const $field_parent = this_field.closest('.orderable-product-fields__field');
      if (!$field_parent) {
        return;
      }
      const is_max_selection_limit = $field_parent.hasClass('orderable-product-fields__field--has-max-selection'),
        max_selection_limit = $field_parent.data('max-selection');
      if (is_max_selection_limit) {
        const selections = $field_parent.find('.orderable-product-option__hidden-field:checked').length;
        if (selections > max_selection_limit) {
          this_field.prop('checked', false);
          jQuery('.orderable-product-fields__max-selection-error').show();
        } else {
          jQuery('.orderable-product-fields__max-selection-error').hide();
        }
      }
      $field_parent.find('.orderable-product-option').each(function () {
        const $option = $(this);
        const $input = $option.find('.orderable-product-option__hidden-field');
        if ($input.is(':checked')) {
     ...
     */
    update_price($field) {
      let $field_group_wrap = $field.closest('.orderable-product-fields-group-wrap'),
        ...
        regular_price = parseFloat($field_group_wrap.data('regular-price')),
  ...
  var field_validator = {
     ...
     */
    on_ready() {
      const debounced_validate = field_validator.debounce(field_validator.validate, 300);
      ...
      $('.orderable-product-fields input, .orderable-product-fields select, .orderable-product-fields textarea').keyup(debounced_validate);
    ...
    validate() {
      $('.orderable-product-fields--required').each(function () {
        const $field_wrap = $(this).hasClass('orderable-product-fields--visual') ? $(this) : $(this).find('select, input, textarea');
        if (!field_validator.validate_field($(this))) {
          $field_wrap.addClass('orderable-field--invalid');
          $('.orderable-drawer .orderable-product__add-to-order, .orderable-drawer .orderable-product__update-cart-item').attr('disabled', 'disabled');
        } else {
    ...
    validate_field($field_wrap) {
      ...
      if ($field_wrap.hasClass('orderable-product-fields--select') || $field_wrap.hasClass('orderable-product-fields--text')) {
        return '' === $field_wrap.find('select, input, textarea').val() ? false : true;
      }
    },
    /**
     * Debounce function.
