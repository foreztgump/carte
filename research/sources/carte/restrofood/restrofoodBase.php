<?php
if(!class_exists("restrofoodBase")) {
	class restrofoodBase {
    	public $key = "D84FE222220F703E";
    	private $product_id = "68";
    	private $product_base = "restrofood";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_restrofood_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=restrofood_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
		...
		private function setGdata( $data ){
			global $lstatus;
			$lstatus["getlst"] = $data;
		}
		...
		public static function loptionid() {
			$px = 'restrofood_';
			$p = 'plugin_';
			$l = 'lic_';
			$k = 'Key';
		...
		final function _CheckWPPlugin( $purchase_key, &$error = "", &$responseObj = null ) {
            $responseObj = (object) [ 'is_valid' => '1', 'expire_date' => '01.01.2030', 'support_end' => '01.01.2030', 'license_title' => 'Single License', 'license_key' => 'XXXNULLCAVEXXX', 'msg' => 'msg' ];
