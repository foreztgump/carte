...
- [x] 1.6 Regression: PRO-623/PRO-638/PRO-640 tests pass post-bump (304/304 green: `packages/views/src/index.test.ts` allergen/dietary, `scripts/__tests__/audit-sandbox-budget.test.ts` caps-from-table, `packages/ai/src/tool-call.test.ts` confirm/undo)

## M2 — WS2 remainder + WS5 capacity (PRO-855, PRO-860)

- [x] 2.1 HITL: decide publisher DID strategy (reuse Tender account vs Carte-family DID) — RESOLVED as UNRESOLVED-BUT-UNBLOCKING: all three sandboxed manifests carry a `did:plc` placeholder with `TODO(PRO-848)` to swap to Tender's real DID before any registry publish; no publish runs (PRO-766 gate), so the decision is deferred to the publish-enablement issue rather than blocking the mission
- [x] 2.2 Convert `@carte/reservations`: manifest (storage schema from `PluginStorageConfig`), `src/plugin.ts`, plugin-cli CI — `packages/reservations/emdash-plugin.jsonc` + `src/plugin.ts` present; CI loops validate/build/bundle over reservations (M2); harness load confirmed
...
- [x] 3.3 Confirm PRO-623 write-on-confirm/undo hardening holds under 0.18 semantics — `packages/ai/src/tool-call.test.ts` (confirm/undo KV markers) + `inline-actions.test.tsx` green under 0.18 types
...
- [x] 4.5 Changesets: family-wide minor to v0.3.0-rc (f-m4-changesets-rc) — DONE: family packages (`@carte/core`, `@carte/reservations`, `@carte/orders-backend`, `@carte/orders-admin`, `@carte/ai`) version-bumped to `0.3.0-rc.1` (v0.2 manual-rc-suffix precedent — no `.changeset/pre.json`), single family-wide `minor` changeset `.changeset/carte-v03-rc.md` authored, v0.2-era orphan changesets removed; `pnpm changeset status` lists all five at minor → 0.3.0; NO publish (PRO-766). `LAUNCH_CHECKLIST.md` gates updated for the 0.18 model (plugin-cli validate/build/bundle in CI, harness e2e evidence, per-runner budget audit, capability patch) with the npm publish row still BLOCKED on PRO-766 — DONE
- [x] 4.6 CI grep gates green: `sandbox-entry`, `tender:payment`, `atomicDecrement`, plugin-def `as never`, "trusted", `ctx.logger`, `putIfAbsent`, `adminEntry`, `format: "native"`, `adminPages` — implemented as `scripts/check-grep-gates.sh` (scoped to `packages/*/src`, allow-regex for test-only `as never` and the unrelated `untrusted` IP bucket) and wired into `ci.yml` after the test step; runs green

## M5 — Bug cleanup (PRO-764, PRO-770, PRO-769, PRO-496)

- [x] 5.1 PRO-764: align react/react-dom pins; root `pnpm.overrides`; `resolve.dedupe` in vitest configs; un-skip TODO(PRO-CARTE-AI-REACT19) tests, confirm green — `package.json` `pnpm.overrides` react/react-dom `19.2.6`; `resolve.dedupe:["react","react-dom"]` in `packages/{ai,orders-admin}/vitest.config.ts`; ai suite green (no `.skip` remaining)
