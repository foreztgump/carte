...
Define how `@carte/orders-backend` learns terminal Tender payment states and
reconciles order lifecycle status, using the published `@tenderpay/sdk`
consumer-eventing API as the only supported channel. Closes the WS4 keystone gap
(PRO-859): an order placed through Tender hosted checkout flips to `paid` exactly
once, idempotent under at-least-once redelivery, entirely in-request.
