...
track.
**Summary:** No open criticals. The one high-priority trust-contract finding
(CAP-002) was resolved in PR #32 (PRO-912); the WAF implementation gap remains
tracked in PRO-911.
...
| CAP-002  | P1       | Capabilities | `packages/orders-backend/emdash-plugin.jsonc:17-20`; `packages/orders-backend/src/routes/checkout.ts:124-135`; `packages/orders-backend/src/routes/refund.ts:162-173`                                                                      | Resolved in PR #32 (PRO-912). The trust contract is reconciled: because the Tender base URL is an operator-configured runtime setting (`tenderBaseUrl`) unknown at authoring time, a static `allowedHosts` allowlist cannot enumerate it. The manifest now declares the canonical `network:request:unrestricted` capability and drops the stale `license.carteplugin.dev` allowlist; manifest tests assert no `allowedHosts` and no Stripe hosts. Card data still never transits Carte. |
...
| `@carte/orders-backend` | Complete. Declared `content:read`, `content:write`, `network:request:unrestricted`; unused `email:send` was removed, and CAP-002/PRO-912 reconciled the Tender host coverage in PR #32 by adopting `network:request:unrestricted` for the runtime-configured Tender base URL. | Complete. Sandbox egress uses `ctx.http.fetch`; CAP-002/PRO-912 was resolved in PR #32 — the runtime-configured Tender base URL (`settings.tenderBaseUrl`) is covered by the canonical `network:request:unrestricted` capability rather than a static `allowedHosts` entry. | N/A      | Complete. Carte consumes Tender-hosted checkout URLs and Tender transaction/refund ids; no raw PAN/CVC observed. | Gap: no WAF config in repo; follow-up needed. |
...
- At audit time the manifest declared `content:read`, `content:write`,
  `network:request`, with `allowedHosts` set to `["license.carteplugin.dev"]`,
  and the manifest test asserted that Stripe hosts and
  `network:request:unrestricted` stayed absent. PR #32 (PRO-912) resolved
  CAP-002 by inverting that contract: the manifest now declares
  `network:request:unrestricted` and drops the `allowedHosts` allowlist, and the
  manifest test now asserts no `allowedHosts` and no Stripe hosts
  (`packages/orders-backend/emdash-plugin.jsonc:17-20`,
  `packages/orders-backend/src/manifest.test.ts:16-26`).
- The audit-time allowlist was non-empty and wildcard-free, but its declared
  host (`license.carteplugin.dev`) did not match any observed orders-backend
  source path; that stale host is the contract mismatch CAP-002 resolved.
...
- Finding CAP-002/PRO-912 (the trust-contract mismatch where observed Tender
  egress ran against `settings.tenderBaseUrl` while the only declared allowlist
  host was `license.carteplugin.dev`) was resolved in PR #32: because the Tender
  base URL is operator-configured at runtime and unknown at authoring time, a
  static `allowedHosts` allowlist cannot enumerate it, so the manifest adopts
  the canonical `network:request:unrestricted` capability and drops the stale
  allowlist.
...
- `@carte/orders-backend`: checkout and refund both require `ctx.http` and pass
  ...
  sandbox egress primitive was observed in non-test sandboxed source. CAP-002
  was resolved in PR #32 (PRO-912): the observed Tender base URL is
  runtime-configured (`settings.tenderBaseUrl`) and unknown at authoring time,
  so the manifest now declares the canonical `network:request:unrestricted`
  capability and no longer pins a static `allowedHosts` entry
  (`packages/orders-backend/emdash-plugin.jsonc:17-20`).
