...
Across the vendored sources, the only obvious schema artifacts are Gutenberg block metadata declarations (`$schema` in block manifests) rather than dedicated restaurant/menu structured-data generation, reinforcing the PRD claim that rich-results support is not a first-class product concern (`research/sources/carte/latepoint/blocks/src/book-form/block.json:2`, `research/sources/carte/orderable-pro/assets/blocks/location-picker/block.json:2`). RestroFood ships no Gutenberg blocks at all and emits zero JSON-LD: a literal grep across `research/sources/carte/restrofood/` for `json-ld`, `application/ld+json`, or `schema.org` returns no PHP/JS hits, and the only header touchpoint is its plain-text "Header text restaurant name" component (`research/sources/carte/restrofood/inc/class-components.php:377`) — i.e., the closest-scope competitor to Carte does not even attempt restaurant structured data.
